...
#Region "Event Control"
    Private Sub trvLop_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles trvPhongThi.AfterSelect
        ...
        If e.Node.Tag <> "" Then
            Dim Arr_he() As String = Split(e.Node.FullPath, "\")
            mTen_he = Arr_he(0)
            ...
            If Arr.Length = 4 Then
                ...
                mLan_thi = Arr(3)
                mID_phong_thi = 0
                Dim Arr_mon() As String = Split(e.Node.Parent.Parent.Text, ":")
                mTen_mon = Arr_mon(1)
            ElseIf Arr.Length = 5 Then
                ...
                mLan_thi = Arr(3)
                mID_phong_thi = Arr(4)
                Dim Arr_mon() As String = Split(e.Node.Parent.Parent.Parent.Text, ":")
                mTen_mon = Arr_mon(1)
...
#Region "Control Events"
    ...
    Private Sub TreeView_Lop(ByVal trvThi As TreeView, ByVal dtThi As DataTable, Optional ByVal SelectAll As Boolean = False)
        ...
        Dim ID_he As Long = 0, ID_Mon As Long = 0, Dot_thi As Byte = 0, Lan_thi As Byte = 0, Tui_thi As Byte = 0, ID_phong_thi As Integer = 0
        Dim j As Integer = -1, k As Integer = -1, l As Integer = -1, m As Integer = -1, n As Integer = -1
...
PhongThi:
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes.Add("Phòng thi: " + dr.Item("Ten_phong").ToString)
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes(n).Tag = dr.Item("ID_he").ToString + "#" + dr.Item("ID_thi").ToString + "#" + dr.Item("ID_mon").ToString + "#" + dr.Item("Lan_thi").ToString + "#" + dr.Item("ID_phong_thi").ToString
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes(n).ImageIndex = 5
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes(n).SelectedImageIndex = 5
                            n += 1
                        Else
                            m = m + 1
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes.Add("Dot thi: " + dr.Item("Dot_thi").ToString)
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).Tag = dr.Item("ID_he").ToString + "#" + dr.Item("ID_thi").ToString + "#" + dr.Item("ID_mon").ToString + "#" + dr.Item("Lan_thi").ToString
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).ImageIndex = 4
                            trvThi.Nodes(j).Nodes(k).Nodes(l).Nodes(m).SelectedImageIndex = 4
                            ID_phong_thi = 0
                            n = 0
                            GoTo PhongThi
                    ...
                    Else
                        l = l + 1
                        trvThi.Nodes(j).Nodes(k).Nodes.Add("Thi lần: " + dr.Item("Lan_thi").ToString)
                        trvThi.Nodes(j).Nodes(k).Nodes(l).Tag = ""
                        trvThi.Nodes(j).Nodes(k).Nodes(l).ImageIndex = 3
                        trvThi.Nodes(j).Nodes(k).Nodes(l).SelectedImageIndex = 3
                        Dot_thi = 0
                        m = -1
                        GoTo DotThi
                ...
                Else
                    k += 1
                    trvThi.Nodes(j).Nodes.Add("Môn: " + dr.Item("Ten_mon").ToString)
                    trvThi.Nodes(j).Nodes(k).Tag = ""
                    trvThi.Nodes(j).Nodes(k).ImageIndex = 2
                    trvThi.Nodes(j).Nodes(k).SelectedImageIndex = 2
                    Lan_thi = 0
                    l = -1
                    GoTo LanThi
            ...
            Else
                ...
                trvThi.Nodes(j).Tag = ""
                trvThi.Nodes.Item(j).ImageIndex = 0
                trvThi.Nodes.Item(j).SelectedImageIndex = 0
                trvThi.Nodes(j).Expand()
                ID_Mon = 0
                k = -1
                GoTo MonHoc
